...
public class ArrayListPracticeLab 
{
	...
	public static int maxLength(ArrayList<String> string_list) {
		int length = 0;
		for(int i = 0; i < string_list.size(); i++) {
			if(string_list.get(i).length() > length) {
				length = string_list.get(i).length();
			}
		}
		return length;
	}
	...
	public static ArrayList swapPairs(ArrayList<String> string_array) {
		ArrayList<String> answer = new ArrayList<String>();
		if(string_array.size() % 2 == 0) {
			for(int i = 0; i < string_array.size(); i+=2) {
				String temp = string_array.get(i);
				answer.add(string_array.get(i + 1));
				answer.add(temp);
			}
		} else {
			for(int i = 0; i < string_array.size() - 1; i+=2) {
				String temp = string_array.get(i);
				answer.add(string_array.get(i + 1));
				answer.add(temp);
			}
		answer.add(string_array.get(string_array.size() - 1));
		}
		return answer;
	}
	...
	public static ArrayList removeEvenLength(ArrayList<String> string_array) {
		ArrayList<String> answer = new ArrayList<String>();
		for(int i = 0; i < string_array.size(); i++) {
			if(string_array.get(i).length() % 2 != 0) {
				answer.add(string_array.get(i));
			}
		}
		return answer;
	}
